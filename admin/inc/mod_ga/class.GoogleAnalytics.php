 ...
 * @author (editor) Peter Jurkovic 2011 
 * @version 0.1.2
 */
  ...
  class GoogleAnalytics{
    ...
    * @return analytics
    */
    public function __construct($sUser , $sPass = false){
    ...
    */
    private function auth(){
        ...
        if ($_SESSION['auth'] == ''){
            unset($_SESSION['auth']);
            throw new GoogleAnalyticsException('Retrieving Auth hash failed!');
        }
        $this->_sAuth = $_SESSION['auth']; 
    }
    ...
    */
    private function getXml($sUrl){
        if($this->_sPass){
            ...
            $aHeader[0] = sprintf("Authorization: AuthSub token=\"%s\"/n", $this->_sAuth );
            return $this->getUrl($sUrl, array(), $aHeader );
        }
    ...
    */
    private function getUrl($sUrl, $aPost = array(), $aHeader = array()){
        ...
        if (function_exists('curl_init')){
            ...
            if ($sOutput === false){
                throw new GoogleAnalyticsException('Curl error (' . curl_error($rRequest) . ')');    
            }
            ...
            if ($aInfo['http_code'] != 200){
                ...
                if ($aInfo['http_code'] == 400){
                    throw new GoogleAnalyticsException('Bad request (' . $aInfo['http_code'] . ') url: ' . $sUrl);     
                }
                if ($aInfo['http_code'] == 403){
                    throw new GoogleAnalyticsException('Access denied (' . $aInfo['http_code'] . ') url: ' . $sUrl);     
                }
                throw new GoogleAnalyticsException('Not a valid response (' . $aInfo['http_code'] . ') url: ' . $sUrl);
            }
            ...
            if (strpos($http_response_header[0], '200') === false){
                // not a valid response from GA   
                throw new GoogleAnalyticsException('Not a valid response (' . $http_response_header[0] . ') url: ' . $sUrl);       
            }
        }
        return $sOutput;
    }  
    ...
    */
    public function setProfileByName($sAccountName){
        ...
        if ($this->_sProfileId == ''){
            throw new GoogleAnalyticsException('No profile ID found!');
        }
